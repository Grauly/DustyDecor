...
interface ShapeDefinition {
    ...
    fun apply(vertexConsumer: VertexConsumer, minUv: Vec2, maxUv: Vec2, extraSetup: (VertexConsumer) -> Unit = {}) {
        ...
        for (i in 0..<getPoints().size) {
            ...
            val vertex = vertexConsumer.addVertex(getPoints()[i].toVector3f())
                .setUv(minUv.x + uv.x * uvMul.x, minUv.y + uv.y * uvMul.y)
            extraSetup.invoke(vertex)
        }
    }

    fun applyMatrix(
        matrixStack: PoseStack.Pose,
        vertexConsumer: VertexConsumer,
        minUv: Vec2,
        maxUv: Vec2,
        extraSetup: (VertexConsumer) -> Unit = {}
    ) {
        val uvDiff = maxUv.add(minUv.negated())
        for (i in 0..<getPoints().size) {
            val uv = getUvs()[i]
            val uvMul = Vec2(uv.x * uvDiff.x, uv.y * uvDiff.y)
            val vertex = vertexConsumer.addVertex(matrixStack, getPoints()[i].toVector3f())
                .setUv(minUv.x + uv.x * uvMul.x, minUv.y + uv.y * uvMul.y)
            extraSetup.invoke(vertex)
        }
    ...
    fun transformPoint(point: Vec3, posOffset: Vec3, scale: Vec3, rotation: Quaternionf): Vec3 {
        return Vec3(
            point.toVector3f().mul(scale.x.toFloat(), scale.y.toFloat(), scale.z.toFloat()).rotate(rotation)
                .add(posOffset.toVector3f())
        )
    }
